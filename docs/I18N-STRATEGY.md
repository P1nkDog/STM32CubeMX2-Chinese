...
- **同一个词两种译法**：`Alternate Function` 译成「复用功能」（对），
  ...
  用户看到会以为是两个不同的概念；
- **同一个词三种译法，而且门禁全绿**（2026-09-22）：`activate` 一族在一屏里
  并存「激活 / 使能 / 启用」三种写法（`Activate`=启用、`Activate HMAC feature`=使能、
  `Activate the HMAC feature.`=激活），当时门禁 108 项通过、0 告警。
  原因是 `terms` 只比对**精确键** —— 钉得住按钮那一条，管不到散文。
  这一条直接催生了 `family` 词族断言（见 11.4）。
...
| `family` | **词族**：英文原文命中正则的条目，译法里不得出现的写法 | `activat\|active` 命中者不得写「激活」 |
...
4. `family` 的词族断言：英文原文命中 `en` 正则的条目，译法里不得出现 `ban` 中的写法。

第 4 条是补第 1 条的短板：`terms` 逐条比对精确键，没人会把 26 条含 `activate`
的散文抄进 `terms`，所以「同一动词三种写法」恰好从它指缝里漏了过去。
词族规则本身也可能失效（英文写法变了、正则写坏了、`ban` 忘填），因此这三种
形状**都被要求出声**：零命中与缺 `ban` 报 Warn，坏正则与空 `en` 报 Error ——
一条会静默跳过的规则，等价于从此永远通过。
`tools/test_glossary.py` 第 12 组把这四种失效形状各钉了一条断言。
