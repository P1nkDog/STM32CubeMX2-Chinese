...
- **语法门禁**：落盘前先用 `node --check` 校验，不通过就整文件不写（可选，没装 Node 时跳过）
...
`node` 只用于落盘前的语法门禁，**不是运行依赖**：缺失时跳过这道校验并打一行普通
提示，汉化照常完成（`--doctor` 能查这道门禁在当前机器上开没开）。
...
**Q: 没装 Node.js 能用吗？**
A: **能，Node 不是运行依赖。** 汉化本身不碰 Node —— 它只用来在写盘前跑一次
   `node --check`，校验注入后的 JS 语法还是不是合法的（这道保险能挡住"注入把
   结构写坏 → CubeMX2 白屏"）。找不到 Node 时工具照常完成汉化，只打一行普通
   提示：`提示: 本机没有 Node.js，已跳过落盘前的语法检查（可选保险，不影响本次汉化）`
   —— 它**不在**「— 问题 —」列表里，也不影响退出码。
   哪台机器上想确认这道保险开没开，跑 `--doctor`，「— 落盘前语法门禁 —」那节会
   直接回答；要用非标准位置的 node，设环境变量 `CUBEMX2ZH_NODE` 指向 `node.exe`。
